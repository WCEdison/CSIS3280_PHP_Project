...
class GroceryProductsDAO
{
    ...
    static function getAllGroceryProductsBySearch(?string $searchTerm = null, ?string $category = null, ?float $minPrice = null, ?float $maxPrice = null)
    {
        $selectProductsBySearch = "SELECT * FROM grocery_products WHERE 1 ";

        if ($searchTerm !== null) {
            $selectProductsBySearch .= " AND product_name LIKE :name";
            $searchTerm = '%' . $searchTerm . '%';
        }

        if ($category !== null) {
            $selectProductsBySearch .= " AND category = :category";
        }

        if ($minPrice !== null && $maxPrice !== null) {
            $selectProductsBySearch .= " AND price BETWEEN :min_price AND :max_price";
        }

        self::$db->query($selectProductsBySearch);

        if ($searchTerm !== null) {
            self::$db->bind(":name", $searchTerm);
        }
